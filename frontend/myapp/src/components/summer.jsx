...
export default function Summer() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // ✅ Filter products by subCategory "summer"
        const filtered = data
          .filter((p) => p.subCategory?.toLowerCase() === "summer")
          .map((product) => ({
            ...product,
            image: formatImageUrl(product.image),
          }));

        setSummerProducts(filtered);
      } catch (error) {
  ...
  if (!summerProducts.length) {
    return <p style={{ textAlign: "center" }}>No products found.</p>;
  }

  return (
    <>
      <Product products={summerProducts} title="Summer Collection" />
    </>
  );
}
